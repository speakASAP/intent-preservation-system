# Copilot Instructions — `intent-preservation-system`
...
- **Authoritative path**: `/home/ssf/Documents/Github/intent-preservation-system` on the `alfares` server.
- **Origin**: `git@github.com:speakASAP/intent-preservation-system.git`, branch `main`.
...
3. This repo, in order: `AGENTS.md`
4. `~/.ai-agent-standards/CROSS_AGENT_AUTOMATION_STANDARD.md` — cross-agent coordination contract

## Deploy

**This repository is DENY-LISTED.** Auto-deploy never touches it — manual deploy is the only way,
and only when the user explicitly asks. Commits to `main` here do **not** ship anything.
...
No Kubernetes deployment in `statex-apps` maps to this repository. It has no runtime of its own.
