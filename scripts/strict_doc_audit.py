...
REQUIRED_GROUPS: list[tuple[str, str, str]] = [
    ("docs/04_systems", "SYS-*.md", "system documents"),
    ("docs/05_subsystems", "SUB-*.md", "subsystem documents"),
    ("docs/07_decisions", "ADR-*.md", "architecture decision records"),
    ("docs/09_milestones", "MS-*.md", "milestone documents"),
    ("docs/10_features", "FEAT-*.md", "feature documents"),
    ("docs/11_tasks", "TASK-*.md", "task documents"),
    ("docs/21_execution_plans", "EP-TASK-*.md", "execution plans"),
    ("docs/22_goal_impact", "GOAL-IMPACT-*.md", "goal impact records"),
]
...
TEMPLATE_FOR_DOC_TYPE: dict[DocType, str] = {
    "SYSTEM": "docs/18_templates/SYSTEM_TEMPLATE.md",
    "SUBSYSTEM": "docs/18_templates/SUBSYSTEM_TEMPLATE.md",
    "ADR": "docs/18_templates/ADR_TEMPLATE.md",
    "FEATURE": "docs/18_templates/FEATURE_TEMPLATE.md",
    "TASK": "docs/18_templates/TASK_TEMPLATE.md",
    "EXECUTION_PLAN": "docs/18_templates/EXECUTION_PLAN_TEMPLATE.md",
    "GOAL_IMPACT": "docs/18_templates/GOAL_IMPACT_TEMPLATE.md",
    "CONTEXT_PACKAGE": "docs/18_templates/CONTEXT_PACKAGE_TEMPLATE.md",
    "CODING_PROMPT": "docs/18_templates/CODING_PROMPT_TEMPLATE.md",
    "VALIDATION_REPORT": "docs/18_templates/VALIDATION_REPORT_TEMPLATE.md",
    "SEMANTIC_COMPRESSION": "docs/18_templates/SEMANTIC_COMPRESSION_TEMPLATE.md",
}
...
def classify(path: Path, root: Path) -> DocType | None:
    ...
    if rel_text.startswith("docs/04_systems/") and name.startswith("SYS-"):
        return "SYSTEM"
    if rel_text.startswith("docs/05_subsystems/") and name.startswith("SUB-"):
        return "SUBSYSTEM"
    if rel_text.startswith("docs/07_decisions/") and name.startswith("ADR-"):
        return "ADR"
    if rel_text.startswith("docs/10_features/") and name.startswith("FEAT-"):
        return "FEATURE"
    if rel_text.startswith("docs/11_tasks/") and name.startswith("TASK-"):
        return "TASK"
    if rel_text.startswith("docs/12_validation/") and name.startswith("VAL-"):
        return "VALIDATION_REPORT"
    if rel_text.startswith("docs/13_context_packages/") and name.startswith("CP-"):
        return "CONTEXT_PACKAGE"
    if rel_text.startswith("docs/14_prompts/") and name.startswith("PROMPT-"):
        return "CODING_PROMPT"
    if rel_text.startswith("docs/21_execution_plans/") and name.startswith("EP-TASK-"):
        return "EXECUTION_PLAN"
    if rel_text.startswith("docs/22_goal_impact/") and name.startswith("GOAL-IMPACT-"):
        return "GOAL_IMPACT"
    if rel_text.startswith("docs/20_semantic_compression/summaries/") and name.endswith(
        ".summary.md"
    ):
        return "SEMANTIC_COMPRESSION"
    if rel_text.startswith("docs/20_semantic_compression/ultra/") and name.endswith(
        ".ultra.md"
    ):
        return "SEMANTIC_COMPRESSION"
    return None
...
def reference_candidates(root: Path, base_path: Path, ref: str) -> list[Path]:
    clean = strip_fragment(ref)
    if not clean:
        return []
    candidates = [
    ...
    if NUMBERED_DIR_RE.match(first) and not clean.startswith(f"{DOCS_ROOT}/"):
        candidates.append((root / DOCS_ROOT / clean).resolve())
    if clean.startswith(f"../{DOCS_ROOT}/"):
        candidates.append((root / clean.removeprefix("../")).resolve())
...
def path_exists(root: Path, base_path: Path, ref: str) -> bool:
    if resolve_reference(base_path, ref) is None and not strip_fragment(ref):
        return True
    ...
    if not clean or clean.startswith("$"):
        return True
    for candidate in reference_candidates(root, base_path, ref):
        try:
...
def audit_context_package_traceability_fields(doc: DocumentInfo) -> list[Finding]:
    ...
    title_task_ids = set(re.findall(r"\bTASK-\d+\b", title or ""))
    if not title or not title.startswith("Context Package:"):
        findings.append(
            make_finding(
                "high",
                "missing_metadata",
                doc.rel,
                f"{doc.rel} title does not declare context package metadata.",
                "Set the top-level title to '# Context Package: <TASK-ID>'.",
                section="Context Package title",
                draft_source=TEMPLATE_FOR_DOC_TYPE["CONTEXT_PACKAGE"],
            )
        )
    elif not title_task_ids:
        findings.append(
            make_finding(
                "high",
                "missing_traceability_field",
                doc.rel,
                f"{doc.rel} title does not identify a TASK id.",
                "Include the target TASK id in the '# Context Package: <TASK-ID>' title.",
                section="Context Package title",
                draft_source=TEMPLATE_FOR_DOC_TYPE["CONTEXT_PACKAGE"],
            )
        )
    ...
    elif title_task_ids and task_ids.isdisjoint(title_task_ids):
        findings.append(
            make_finding(
                "high",
                "traceability_mismatch",
                doc.rel,
                f"{doc.rel} title task id does not match the Target task section.",
                "Use the same TASK id in the package title and Target task section.",
                section="Target task",
                draft_source=TEMPLATE_FOR_DOC_TYPE["CONTEXT_PACKAGE"],
            )
        )
    if not any(
        ref.startswith(("docs/11_tasks/", "../11_tasks/", "../docs/11_tasks/"))
        for ref in extract_references(target_task)
    ):
        findings.append(
            make_finding(
                "high",
...
def extract_included_document_references(text: str) -> set[str]:
    refs = set(extract_references(text))
...
def audit_link_reality(root: Path, docs: list[DocumentInfo]) -> list[Finding]:
    ...
    for doc in docs:
        ...
        if doc.doc_type == "CONTEXT_PACKAGE":
            for ref in sorted(extract_included_document_references(section_body(doc, "Included documents"))):
                if not path_exists(root, doc.path, ref):
                    findings.append(
                        make_finding(
                            "high",
