...
CONTEXT_PACKAGE_DIR = "docs/13_context_packages"
...
def task_path_for(root: Path, task_id: str) -> Path:
    matches = sorted((root / "docs/11_tasks").glob(f"{task_id}*.md"))
    if not matches:
...
def to_package_rel(root: Path, root_rel: str) -> str:
    """Convert a repo-root-relative path into a path relative to the context-package dir."""
    if root_rel.startswith(("http://", "https://", "#")):
        return root_rel
    clean, fragment = root_rel.split("#", 1) if "#" in root_rel else (root_rel, "")
    rel = Path(
        os.path.relpath((root / clean).as_posix(), (root / CONTEXT_PACKAGE_DIR).as_posix())
    ).as_posix()
    if fragment:
...
def root_rel_from_task(root: Path, task_path: Path, ref: str) -> str:
    if ref.startswith(("http://", "https://", "#")):
        return ref
    clean, fragment = ref.split("#", 1) if "#" in ref else (ref, "")
...
def normalize_rel_from_task(root: Path, task_path: Path, ref: str) -> str:
    return to_package_rel(root, root_rel_from_task(root, task_path, ref))
...
def repository_rel(root: Path, ref: str) -> str:
    clean = ref.split("#", 1)[0]
    package_dir = root / CONTEXT_PACKAGE_DIR
    candidates = [
        (package_dir / clean).resolve(),
        (root / clean).resolve(),
    ]
    stripped = clean
    ...
    if re.match(r"\d{2}_", stripped):
        candidates.append((root / "docs" / stripped).resolve())
...
{task.task_id}: `{to_package_rel(root, task.task_rel)}`
...
- Do not modify `{to_package_rel(root, "docs/00_constitution/CONSTITUTION.md")}` or `{to_package_rel(root, "docs/01_vision/VISION.md")}`.
...
def output_path_for(root: Path, task_id: str) -> Path:
    ...
    return root / CONTEXT_PACKAGE_DIR / f"CP-{suffix}.md"
